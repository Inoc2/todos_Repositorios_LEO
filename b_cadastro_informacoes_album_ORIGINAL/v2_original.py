#Otimizar o código, mudar nome das variáveis/funções e incluir funções de verificações
from tkinter import *
...
def main_features():
    ...
    main_title_label.place(x=95, y=5)
...
def saving_the_album():
    ...
    while True:
        with open('registered_albums.txt', 'r', encoding='UTF-8') as archive_txt:
            ...
            if '' in current_album:
                status_saving_album_label("Você ainda não preencheu todos os campos necessários. Tente novamente.", current_height + 40, "red")
                break
            
            try:
                current_album[1] = int(current_album[1])
            except Exception:
                status_saving_album_label("Você inseriu um ano inválido. Tente novamente.", current_height + 40, "red")
                break
            
            try:
                for line_archive_txt in list_from_archive_txt:
                    if line_archive_txt.split(",")[0] == current_album[0]:
                        status_saving_album_label("Você inseriu um álbum já existente no programa. Tente novamente.", current_height + 40, "red")
                        break
                if line_archive_txt.split(",")[0] == current_album[0]:
                    break
            except:
    ...
    while True:
        with open('registered_albums.txt', 'r', encoding='UTF-8') as archive_txt:
            ...
            if current_album_from_all_albums_searching_list[0] == 1:
                search_all_albums_tree.delete(*search_all_albums_tree.get_children())
                for line_archive_txt in list_from_archive_txt:
                    line_archive_txt = line_archive_txt.split(",")
                    search_all_albums_tree.insert('', 'end', values=(line_archive_txt[0], line_archive_txt[1], line_archive_txt[2], "Não" if line_archive_txt[3] == "1\n" else "Sim"))
                break
...
def search_all_albums_tree():
    global search_all_albums_tree
    columns_search = ('Nome do Álbum', 'Ano', 'Nome(s)', 'Lançamento')

    search_all_albums_tree = ttk.Treeview(main_window, columns=columns_search, show='headings')

    search_all_albums_tree.heading('Nome do Álbum', text='Nome do Álbum')
    search_all_albums_tree.heading('Ano', text='Ano')
    search_all_albums_tree.heading('Nome(s)', text='Nome(s)')
    search_all_albums_tree.heading('Lançamento', text='Lançamento')

    search_all_albums_tree.grid(row=0, column=0, sticky='nsew')
    search_all_albums_tree.place(x=200, y=450, width=510, height=290)

    search_all_albums_tree.column('Nome do Álbum', width=180)
    search_all_albums_tree.column('Ano', width=70)
    search_all_albums_tree.column('Nome(s)', width=150)
    search_all_albums_tree.column('Lançamento', width=70)
